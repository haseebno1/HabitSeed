...
// Cache implementation for Capacitor storage
...
/**
 * CapacitorAdapter implements the StorageAdapter interface using Capacitor Preferences API
 */
export class CapacitorAdapter implements StorageAdapter {
  private cache = new CapacitorCache();
  
  /**
   * Check if Capacitor is supported in this environment
   */
  isSupported(): boolean {
  ...
  async getAllHabits(): Promise<Habit[]> {
    return StorageUtils.executeWithRetry('get all habits', async () => {
      ...
      const habits = result.value ? JSON.parse(result.value) : [];
  ...
  async saveCompletionDetails(date: string, completions: HabitCompletion[]): Promise<void> {
    return StorageUtils.executeWithRetry('save completion details', async () => {
      await Preferences.set({
        key: `${DETAILED_COMPLETIONS_PREFIX}${date}`,
  ...
  async getCompletionDetails(date: string): Promise<HabitCompletion[]> {
    return StorageUtils.executeWithRetry('get completion details', async () => {
      ...
      const result = await Preferences.get({ key: `${DETAILED_COMPLETIONS_PREFIX}${date}` });
      
      if (result.value) {
        const completions = JSON.parse(result.value);
  ...
  async getAllCompletionDetails(): Promise<Record<string, HabitCompletion[]>> {
    return StorageUtils.executeWithRetry('get all completion details', async () => {
      ...
      // Filter for detailed completion keys
      const detailedCompletionKeys = keys.filter(key => key.startsWith(DETAILED_COMPLETIONS_PREFIX));
      
      // Create completions object
      const completions: Record<string, HabitCompletion[]> = {};
      
      // Populate completions object with detailed data
      for (const key of detailedCompletionKeys) {
        const result = await Preferences.get({ key });
        if (result.value) {
          const date = key.replace(DETAILED_COMPLETIONS_PREFIX, '');
          const parsedCompletions = JSON.parse(result.value);
          completions[date] = parsedCompletions;
          this.cache.setCompletionsInCache(date, parsedCompletions);
        }
      }
      
      // If we have detailed data for all dates, return it
      if (detailedCompletionKeys.length > 0) {
  ...
  async migrateFromLocalStorage(): Promise<boolean> {
    return StorageUtils.executeWithRetry('migrate from localStorage', async () => {
      try {
        // Check if we've already migrated
        const migrationFlag = localStorage.getItem("habitSeed_capacitor_migration_completed");
        ...
        localStorage.setItem("habitSeed_capacitor_migration_completed", "true");
  ...
  /**
   * Close the connection (no-op for Capacitor)
   */
  closeConnection(): void {
    // No connection to close in Capacitor
  }
} 